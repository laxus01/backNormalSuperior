...
const getPracticesByGroup = async (req, res) => {
  ...
  db.query(
    "SELECT sa.id, i.id AS institucion_id, i.institucion, sd.sede, j.jornada, g.grado, d.grupo, sa.estudiante_id, dc.nombre AS profesor, e.nombre AS estudiante, sp.id AS supervisor_id, sp.supervisor, tp.tipo, CONCAT(sm.semestre,' ',gr.grupo) AS grupoxsemestre, gr.id AS grupo_id FROM solicitudes s, solicitudes_asignadas sa, estudiantes e, sedes sd, docentes dc, instituciones i, jornadas j, detalle_grupoc d, grados g, grupos gr, semestres sm, tipopractica tp, supervisores sp, matriculas_periodo m WHERE s.id = sa.solicitud_id AND sa.estudiante_id = e.id AND sa.estado = '1' AND sa.tipopractica_id = tp.id AND s.detallegrupoc_id = d.id AND d.grado_id = g.id AND s.jornada_id = j.id AND s.sede_id = sd.id AND sd.institucion_id = i.id AND s.docente_id = dc.id AND sd.supervisor_id = sp.id AND e.id = m.estudiante_id AND m.periodo_id = '6' AND m.grupo_id = gr.id AND gr.semestre_id = sm.id AND sa.periodo_id = '6' AND gr.id = ? ",
    [id],
    (err, rows) => {
      if (err) console.log(err);
      if (err)
...
const getPracticesByInstitution = async (req, res) => {
  ...
  db.query(
    "SELECT sa.id, i.id AS institucion_id, i.institucion, sd.sede, j.jornada, g.grado, d.grupo, sa.estudiante_id, dc.nombre AS profesor, e.nombre AS estudiante, sp.id AS supervisor_id, sp.supervisor, tp.tipo, CONCAT(sm.semestre,' ',gr.grupo) AS grupoxsemestre, gr.id AS grupo_id FROM solicitudes s, solicitudes_asignadas sa, estudiantes e, sedes sd, docentes dc, instituciones i, jornadas j, detalle_grupoc d, grados g, grupos gr, semestres sm, tipopractica tp, supervisores sp, matriculas_periodo m WHERE s.id = sa.solicitud_id AND sa.estudiante_id = e.id AND sa.estado = '1' AND sa.tipopractica_id = tp.id AND s.detallegrupoc_id = d.id AND d.grado_id = g.id AND s.jornada_id = j.id AND s.sede_id = sd.id AND sd.institucion_id = i.id AND s.docente_id = dc.id AND sd.supervisor_id = sp.id AND e.id = m.estudiante_id AND m.periodo_id = '6' AND m.grupo_id = gr.id AND gr.semestre_id = sm.id AND sa.periodo_id = '6' AND i.id = ? ",
    [id],
    (err, rows) => {
      if (err) console.log(err);
      if (err)
...
const getListStudentsAvailable = async (req, res) => {
  db.query(
    "SELECT e.id, e.nombre, CONCAT(s.semestre,' ',g.grupo) AS grupo, s.id AS semestre_id, e.telefono, m.periodo_id FROM estudiantes e, matriculas_periodo m, semestres s, grupos g, periodos p WHERE m.periodo_id = p.id AND e.id = m.estudiante_id AND m.grupo_id = g.id AND g.semestre_id = s.id AND p.id = '6' AND e.id NOT IN (SELECT sa.estudiante_id FROM solicitudes s, solicitudes_asignadas sa WHERE s.id = sa.solicitud_id AND sa.estado = '1') ORDER BY e.nombre ASC ",
    (err, rows) => {
      if (err) console.log(err);
      if (err)
        return res
...
const getConsolidateRecords = async (req, res) => {
  db.query(
    "SELECT e.id, e.nombre, CONCAT(sm.semestre,' ',g.grupo) AS grupo, (SELECT n.nota FROM notas_practicas n, solicitudes_asignadas s WHERE s.id = n.solicitud_id AND n.juicio_id = '1' AND s.estudiante_id = e.id AND n.periodo_id = '6' ORDER BY n.id DESC LIMIT 1) AS nota_ppi, (SELECT n.nota FROM notas_practicas n, solicitudes_asignadas s WHERE s.id = n.solicitud_id AND n.juicio_id = '2' AND s.estudiante_id = e.id AND n.periodo_id = '6' ORDER BY n.id DESC LIMIT 1) AS nota_investigacion, (SELECT n.nota FROM notas_practicas n, solicitudes_asignadas s WHERE s.id = n.solicitud_id AND n.juicio_id = '3' AND s.estudiante_id = e.id AND n.periodo_id = '6' ORDER BY n.id DESC LIMIT 1) AS nota_maestrotitular, (SELECT n.nota FROM notas_practicas n, solicitudes_asignadas s WHERE s.id = n.solicitud_id AND n.juicio_id = '4' AND s.estudiante_id = e.id AND n.periodo_id = '6' ORDER BY n.id DESC LIMIT 1) AS nota_supervisor FROM estudiantes e, matriculas_periodo m, grupos g, semestres sm WHERE m.grupo_id = g.id AND g.semestre_id = sm.id AND e.id = m.estudiante_id AND m.periodo_id = '6' AND e.estado = '1' ORDER BY sm.semestre, g.grupo, e.nombre",
    (err, rows) => {
      if (err) console.log(err);
      console.log(err);
      if (err)
...
const getConsolidateRecordsByGroup = async (req, res) => {
  ...
  db.query(
    "SELECT e.id, e.nombre, CONCAT(sm.semestre,' ',g.grupo) AS grupo, (SELECT n.nota FROM notas_practicas n, solicitudes_asignadas s WHERE s.id = n.solicitud_id AND n.juicio_id = '1' AND s.estudiante_id = e.id AND n.periodo_id = '6' ORDER BY n.id DESC LIMIT 1) AS nota_ppi, (SELECT n.nota FROM notas_practicas n, solicitudes_asignadas s WHERE s.id = n.solicitud_id AND n.juicio_id = '2' AND s.estudiante_id = e.id AND n.periodo_id = '6' ORDER BY n.id DESC LIMIT 1) AS nota_investigacion, (SELECT n.nota FROM notas_practicas n, solicitudes_asignadas s WHERE s.id = n.solicitud_id AND n.juicio_id = '3' AND s.estudiante_id = e.id AND n.periodo_id = '6' ORDER BY n.id DESC LIMIT 1) AS nota_maestrotitular, (SELECT n.nota FROM notas_practicas n, solicitudes_asignadas s WHERE s.id = n.solicitud_id AND n.juicio_id = '4' AND s.estudiante_id = e.id AND n.periodo_id = '6' ORDER BY n.id DESC LIMIT 1) AS nota_supervisor FROM estudiantes e, matriculas_periodo m, grupos g, semestres sm WHERE m.grupo_id = g.id AND g.semestre_id = sm.id AND e.id = m.estudiante_id AND m.grupo_id = ? AND m.periodo_id = '6' AND e.estado = '1' ORDER BY sm.semestre, g.grupo, e.nombre",
    [grupo_id],
    (err, rows) => {
      if (err) console.log(err);
      console.log(err);
